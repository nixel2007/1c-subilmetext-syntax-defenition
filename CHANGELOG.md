...
* [VSC] В шаблоне задач для запуска OneScript расширена обработка вложенных сообщений об ошибках
* [VSC] Автодополнение по стандартным перечислениям теперь работает не только после знака `=`
* [VSC] Добавлено автодополнение по значениям стандартных перечислений
...
* [VSC] Перевод строки внутри `()` теперь добавляет отступ
* [VSC] Автовставка закрывающей скобки теперь учитывает настройку `editor.autoClosingBrackets` и работает только в языке `1C (BSL)`

## 1.8.1 [VSC] / 1.6.1 [ST, Atom]
...
* [VSC] Исправлено отображение подсказки по параметрам процедур для VSC 1.6
* [VSC] Исправлен переход по ссылкам Синтакс-помощника в VSC 1.6
...
## 1.7.0 [VSC] / 1.5.0 [ST, Atom]

* Добавлена возможность автоматически разворачивать конструкции вида `++`/`+=` и подобных по нажатию на `Tab`
* [VSC] Добавлено автоформатирование текста по `Shift-Alt-F`
* [Atom] Добавлена комбинация горячих клавиш `Shift-Alt-F` для автоформатирования кода
* [Atom] Добавлена поддержка подсветки TODO/FIXME/HACK и прочих сочетаний внутри комментариев
* [VSC] Исправлено отображение секции `Примеры` в синтакс-помощнике
...
* Добавлены автодополнение, подсказка при наведении, поддержка подсказки параметров при вводе для процедур и функций OneScript
...
* [Atom] Добавлен переход к строке с ошибкой при использовании пакета build
* В варианты запуска скриптов oscript добавлен режим `-check`
...
* [ST] Исправлена ошибочная вставка перевода строки и символа `|` при открытом окне автодополнения
