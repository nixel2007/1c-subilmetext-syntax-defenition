...
Процедура СобратьГрамматикуЯзыка(Знач ИмяФайлаЯзыка)
	...
	ПутьКФайлуYAML = ИмяФайлаЯзыка + ".YAML-tmLanguage";
	Ожидаем.Что(Новый Файл(ПутьКФайлуYAML).Существует(), "Ожидаем, что файл языка существует").ЭтоИстина();
		
	ПутьКФайлуYAML2JSON = ОбъединитьПути(ПутьКБинарникамНод, "yaml2json");
	ПутьКФайлуJSON = ОбъединитьПути(КаталогСборки, ИмяФайлаЯзыка + ".json");	
	КомандаЗапуска = "%1 --pretty %3 > %2";
	КомандаЗапуска = СтрШаблон(КомандаЗапуска, ПутьКФайлуYAML2JSON, ПутьКФайлуJSON, ПутьКФайлуYAML);
	ВыполнитьКоманду(КомандаЗапуска, "Ошибка компиляции YAML -> JSON");
	
	ПутьКФайлуJSON2CSON = ОбъединитьПути(ПутьКБинарникамНод, "json2cson");
	ПутьКФайлуCSON = ОбъединитьПути(КаталогСборки, ИмяФайлаЯзыка + ".cson");
	КомандаЗапуска = "%1 %2 > %3 --2spaces";
	КомандаЗапуска = СтрШаблон(КомандаЗапуска, ПутьКФайлуJSON2CSON, ПутьКФайлуJSON, ПутьКФайлуCSON);
...
Процедура ВыполнитьКоманду(Знач КомандаЗапуска, Знач ТекстОшибки = "", Знач РабочийКаталог = "")
	...
	Если ЭтоWindows = Ложь Тогда 
		СтрокаЗапуска = "sh -c '" + КомандаЗапуска + "'";
	Иначе
		СтрокаЗапуска = "cmd.exe /C " + ОбернутьВКавычки(КомандаЗапуска);
	КонецЕсли;
